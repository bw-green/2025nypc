...
public:
    ...
    vector<vector<int>> buildOponentArea(const vector<vector<int>>& b) {
        vector<vector<int>> Area(ROWS+1, vector<int>(COLS+1, 0));
        for (int r = 0; r < ROWS; ++r)
            for (int c = 0; c < COLS; ++c){
                int temp=b[r][c];
    ...
    vector<int> calculateMove(int time) {
        ...
        for (int r1 = 0; r1 < ROWS; ++r1) for (int r2 = r1; r2 < ROWS; ++r2)
            for (int c1 = 0; c1 < COLS; ++c1) for (int c2 = c1; c2 < COLS; ++c2) {
                ...
                if(banned[r1][c1][r2][c2]) continue;
                bool ok = true;
        ...
        for (auto &c : candidates) {
            ...
            auto area = buildOponentArea(atemp);
            ...
            for (int c1 = 0; c1 < COLS; ++c1) for (int c2 = c1; c2 < COLS; ++c2) {
                ...
                if (!checkEdge(r1, c1, r2, c2)) continue;
                int nowarea=getSum(Area0,r1, c1, r2, c2);
                if(nowarea>totalGain){
                    banned[r1][c1][r2][c2]=true;
                    goto exit_loops;
                }
            }
            bestRect.c1=c.c1;
...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        if (cmd == "READY") {
            ...
            int myT, oppT;
            iss >> myT >> oppT;
            auto mv = game.calculateMove(myT);
