...
struct Game {
    ...
    Move chooseMove(int timeLeft) {
        vector<Move> all = getCandidates(me);
        vector<Move> opAll = getCandidates(opp);

        int myCnt = (int)all.size();
        int opCnt = (int)opAll.size();
        ...
        if (myCnt <= 26 && opCnt <= 26 && timeLeft >= 5000) {
            depth = 7;
            beam = 12;
        }

        if (myCnt <= 18 && opCnt <= 18 && timeLeft >= 3000) {
            depth = 8;
            beam = 10;
        }

        if (myCnt <= 10 && opCnt <= 10 && timeLeft >= 1500) {
            depth = 9;
            beam = 8;
        }

        if (myCnt <= 6 && opCnt <= 6 && timeLeft >= 800) {
            depth = 10;
            beam = 7;
        }
        ...
        for (const Move& m : rootMoves) {
            ...
            if (sc > bestScore || (sc == bestScore && m.orderScore > best.orderScore)) {
                bestScore = sc;
                best = m;
            }
        }
