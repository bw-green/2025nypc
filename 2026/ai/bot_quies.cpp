...
struct Game {
    ...
    // 정적탐색(quiescence): 탈취 수(상대 칸을 먹는 stolen>0)만 따라 교환이 끝날 때까지 더 봄.
    //   "놓자마자 되먹히는" 함정 제거. stand-pat(현 평가)으로 더 안 둘 선택지도 포함.
    long long qsearch(int player, long long alpha, long long beta, int qd) const {
        ...
        vector<Move> cands = getCandidates(player);   // orderScore 정렬(gain 우선)
        long long best = stand;
        int cnt = 0;
        for (const Move& m : cands) {
            if (m.stolen <= 0) continue;              // 탈취 수만(2배 교환)
            Game nx = *this;
            nx.applyMove(m, player);
            long long sc = -nx.qsearch(3 - player, -beta, -alpha, qd - 1);
            if (sc > best) best = sc;
            if (sc > alpha) alpha = sc;
            if (alpha >= beta) break;
            if (++cnt >= 4) break;                    // 분기 제한
        }
        return best;
    }
    ...
    long long negamax(
        ...
        if (depth <= 0 || outOfTime()) {
            return state.qsearch(player, alpha, beta, 4);   // 잎에서 탈취 교환 정적탐색
        }
