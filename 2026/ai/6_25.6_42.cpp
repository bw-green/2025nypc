...
// ================== 학습 가능한 시간 관리 ==================
//  이번 수에 쓸 시간 비율 = clamp(sigmoid(bias + Σ w_i*feat_i), MINF, MAXF) × 남은시간.
//  feat: [진행도, 상대위협(상대 best gain), 내 후보수, |점수차|]  → "언제 더 생각할지"를 학습.
static const double TIME_MINF = 0.03, TIME_MAXF = 0.45;
struct TimeWeights { double bias, w_prog, w_threat, w_cand, w_margin; };
//  기본값(A): 균등 ≈ timeLeft/8 (bias만, 가중치 0 → frac≈0.125). 학습 시 가중치가 채워짐.
static TimeWeights DEFAULT_TIME = { -1.23, 0.0, 0.0, 0.0, 0.0 };
static TimeWeights G_TIME = DEFAULT_TIME;
...
// ===== Transposition Table (같은 시간에 더 깊이 탐색) =====
static const int TTBITS=21;
static const size_t TTSIZE=(size_t)1<<TTBITS, TTMASK=TTSIZE-1;
struct TTEntry{ uint64_t key=0; long long value=0; int16_t depth=-1; uint8_t flag=0; int8_t r1=-1,c1=-1,r2=-1,c2=-1; };
static vector<TTEntry> TT;
static uint64_t ZOB[10][17][3], ZTURN, ZPASS;
static Move KILLER[64][2];   // 깊이별 killer move(컷오프 낸 수) 2개
static void initZobrist(){
    mt19937_64 rng(0x9E3779B97F4A7C15ULL);
    for(int r=0;r<10;r++)for(int c=0;c<17;c++){ ZOB[r][c][0]=0; ZOB[r][c][1]=rng(); ZOB[r][c][2]=rng(); }
    ZTURN=rng(); ZPASS=rng();
}
...
struct Game {
    ...
    int myTurns=0;                 // 내가 둔 횟수(시간관리 prog feature용)
    uint64_t hash=0;               // owner 구성 Zobrist 해시(증분)
    ...
    void init(const vector<string>& rows){
        memset(owner,0,sizeof(owner)); lastPass=false; myTurns=0; hash=0;
    ...
    long long negamax(Game st,int player,int depth,bool prevPass,int beam,long long alpha,long long beta) const {
        long long a0=alpha;
        uint64_t key=st.hash ^ (player==2?ZTURN:0) ^ (prevPass?ZPASS:0);
        TTEntry& e=TT[key&TTMASK]; bool hit=(e.key==key);
        ...
        int front=0; int dk=depth&63;
        if(hit && e.r1>=0){  // TT 최선수 우선
            for(size_t i=0;i<moves.size();i++)
                if(moves[i].r1==e.r1&&moves[i].c1==e.c1&&moves[i].r2==e.r2&&moves[i].c2==e.c2){ swap(moves[front],moves[i]); front++; break; }
        }
        for(int ki=0;ki<2;ki++){  // killer move 우선
            const Move&k=KILLER[dk][ki]; if(k.r1<0)continue;
            for(size_t i=front;i<moves.size();i++)
                if(moves[i].r1==k.r1&&moves[i].c1==k.c1&&moves[i].r2==k.r2&&moves[i].c2==k.c2){ swap(moves[(size_t)front],moves[i]); front++; break; }
        }
        long long best=LLONG_MIN/4; Move bm=st.passMove(); int nb=max(5,beam-2);
        for(const Move&m:moves){
            ...
            if(sc>alpha)alpha=sc;
            if(alpha>=beta){
                if(!m.isPass()){ Move&k0=KILLER[dk][0];
                    if(!(k0.r1==m.r1&&k0.c1==m.c1&&k0.r2==m.r2&&k0.c2==m.c2)){ KILLER[dk][1]=k0; KILLER[dk][0]=m; } }
                break;
            }
            if(timeUp)break;
        }
        if(!timeUp && (!hit||e.depth<=depth)){
            e.key=key; e.value=best; e.depth=(int16_t)depth;
            e.flag=(best<=a0)?2:(best>=beta)?1:0;
            e.r1=(int8_t)bm.r1; e.c1=(int8_t)bm.c1; e.r2=(int8_t)bm.r2; e.c2=(int8_t)bm.c2;
        }
        return best;
    }
    Move chooseMove(int timeLeft){
        auto t0=chrono::steady_clock::now();
        for(int i=0;i<64;i++){ KILLER[i][0]=Move{}; KILLER[i][1]=Move{}; }   // killer 초기화
        // 학습된 시간관리: 상태 feature 로 이번 수 시간비율 결정 (언제 더 생각할지 학습).
        int myCntNow=(int)getCandidates(me).size();
        vector<Move> ocNow=getCandidates(opp);
        double prog   = min(1.0, myTurns/20.0);
        double threat = min(1.0, (ocNow.empty()?0.0:(double)ocNow[0].gain)/12.0);  // 상대 최대 이득(위협)
        double cand   = min(1.0, myCntNow/40.0);
        double margin = min(1.0, fabs((double)diffScore(me))/20.0);
        double z = G_TIME.bias + G_TIME.w_prog*prog + G_TIME.w_threat*threat
                 + G_TIME.w_cand*cand + G_TIME.w_margin*margin;
        double frac = TIME_MINF + (TIME_MAXF-TIME_MINF)/(1.0+exp(-z));
        long long budget = (long long)(timeLeft * frac);
        budget = min<long long>(budget, (long long)timeLeft - 300);
        ...
        Move best=root[0];
        myTurns++;   // prog feature(시간관리)용. 깊이 상한은 없음(A 복귀).
...
static void loadTimeWeights(const char* path){   // 5개: bias w_prog w_threat w_cand w_margin
    ifstream f(path); if(!f) return;
    double a,b,c,d,e;
    if(f>>a>>b>>c>>d>>e){ G_TIME={a,b,c,d,e}; }
}
int main(int argc,char**argv){
    ios::sync_with_stdio(false); cin.tie(nullptr);
    if(const char* d=getenv("FIXED_DEPTH")) FIXED_DEPTH=atoi(d);
    G_WEIGHTS = DEFAULT_WEIGHTS; G_TIME = DEFAULT_TIME;
    if(argc>1) loadWeights(argv[1]);       // argv[1]: 후보 평가 weight
    if(argc>2) loadTimeWeights(argv[2]);   // argv[2]: 시간관리 weight (학습용)
    initZobrist(); TT.assign(TTSIZE, TTEntry{});
    Game game; string cmd;
    while(cin>>cmd){
        if(cmd=="READY"){string o;cin>>o;if(o=="FIRST"){game.me=1;game.opp=2;}else{game.me=2;game.opp=1;}cout<<"OK"<<endl;}
        else if(cmd=="INIT"){vector<string> rows(10);for(int i=0;i<10;i++)cin>>rows[i];game.init(rows); fill(TT.begin(),TT.end(),TTEntry{});}
