...
struct Game {
    ...
    Move applyGreedyResponseGuard(const vector<Move>& rootMoves, const Move& currentBest) const {
        ...
        bool guardOpensBiggerChain =
            guard.oppGain >= guard.move.gain + 4 &&
            guard.oppStolen >= guard.move.stolen + 2;
        bool guardHasChainMargin =
            !guardOpensBiggerChain ||
            guardGain >= 4 ||
            guard.afterDiff >= cur.afterDiff + 4;

        if (directRecaptureUpgrade &&
            modestImmediateCost &&
            respectsSearch &&
            guardHasChainMargin) {
            return guard.move;
